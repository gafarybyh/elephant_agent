            ...
            var columns = dt
                ...
                .map(function (i) {
                    ...
                    return {
                        ...
                        auto: false,
                        control: false,
                        never: className.match(/\b(dtr\-)?never\b/)
                            ? true
                            : false,
                        priority: priority,
                    };
            ...
            if (this.c.details.renderer._responsiveMovesNodes) {
                ...
                dt.rows({ page: "current" }).every(function (rowIdx) {
                    ...
                    dt.cells(rowIdx, visibleColumns).every(function (
                        ...
                        var store =
                            that.s.childNodeStore[rowIdx + "-" + colIdx];
                ...
                $(clonedBody)
                    .append(
                        $(dt.rows({ page: "current" }).nodes()).clone(false)
                    )
                    .find("th, td")
                    .css("display", "");
            }
        ...
        _setHeaderVis: function (col, showHide, structure) {
            ...
            structure.forEach(function (row) {
                if (row[col]) {
                    ...
                    while (search >= 0) {
                        if (row[search]) {
                            row[search].cell.colSpan = that._colspan(
                                row,
                                search
                            );
                            break;
        ...
        modal: function (options) {
            return function (row, update, render, closeCallback) {
                ...
                if (!update) {
                    ...
                    if (
                        modal.length &&
                        row.index() === modal.data("dtr-row-idx")
                    ) {
